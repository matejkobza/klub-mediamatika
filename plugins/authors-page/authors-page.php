	...
	*/
	...
	function uvd_aup_render_authors_page( $role = null )
	{
		...
		foreach ( $authors as $author) {
			$columns_count++;
			if ( $columns_count % $max_column_count == 4 ) {
				$html .= '<tr style="'.$clear_css.'" >';
			}

			$html .= '<td style="'.$clear_css.' width:25%; text-align:center; padding-bottom:1em;">';
			//$html .= '<a href="'.$Permalink.'?author_login='.$author->user_login.'" >' .get_avatar($author->ID).'<br>'.$author->display_name. $post_count->count_posts.'</a></td>';
			$html .= '<a href=" '.get_author_posts_url( $author->ID ).'">'.get_avatar($author->ID).'<br>'.$author->display_name.'' . $post_count . ' </a></td>';
